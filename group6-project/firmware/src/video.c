...
volatile uint32_t *MEDIUM_SPRITE_CONTROLS[128];
...
uint32_t generateMediumSpriteConfig(uint32_t x, uint32_t y, uint32_t w, uint32_t h, uint32_t p){
    return ((h-17)<<26) | ((w-17)<<21) | ((y+32)<<12) | ((x+32)<<2) | p;
}
...
void drawRectangleWithMediumSprite(int sprite_id, uint32_t addr){
    *MEDIUM_SPRITE_CONTROLS[sprite_id] = addr;
}
...
void moveMediumSprite(int sprite_id, uint32_t x, uint32_t y){
    *MEDIUM_SPRITE_CONTROLS[sprite_id] &= 0xFFE00003;
    *MEDIUM_SPRITE_CONTROLS[sprite_id] |= (((y+32)<<12) | ((x+32)<<2));
}
...
void setMediumSpriteColor(int palette_id, int entry_id, uint32_t rgba){
    volatile uint32_t *SPRITE_PALETTE = (volatile uint32_t *)(0x500F2000 + 1024 * palette_id);
    SPRITE_PALETTE[entry_id] = rgba;
}

void setLargeSpriteColor(int palette_id, int entry_id, uint32_t rgba){
    volatile uint32_t *SPRITE_PALETTE = (volatile uint32_t *)(0x500F1000 + 1024 * palette_id);
    SPRITE_PALETTE[entry_id] = rgba;
}
...
void initializeSpriteControllers(){
    ...
    for (int i = 0; i < 128; i++){
        MEDIUM_SPRITE_CONTROLS[i] = (volatile uint32_t *)(0x500F5F00 + i * 4);
    }
    for (int i = 0; i < 64; i++){
...
uint32_t getMediumSpriteControl(int sprite_id){
    return *MEDIUM_SPRITE_CONTROLS[sprite_id];
}
