...
#define THREAD_STACK_SIZE 0x00020000
...
void initialize_threads() {
  for (uint16_t i=0; i<max_threads; ++i) {
    ...
    threads_status[i].wait_for_video = 0;
  }
  next_thread_id = 0;  
...
void wait_for_video() {
  threads_status[current_thread_slot].wait_for_video = 1;
}
...
/*
We access threads in round robin fashion
- To get the next one, we loop through all threads and find the next one ready to run
    - It is ready if:
        - Not an event handler (will get called elsewhere)
	- Has been initialized
	- Is not waiting for time (i.e wait())
	- Is not waiting for another thread to finish
- If not threads are active (different from ready to run), we return an error code
...
uint16_t get_next_thread_slot() {
  ...
  while (1) {
    ...
    if (thread_to_try->id != -1 && !thread_to_try->event_handler) {
	  ...
	  (!thread_to_try->wait_for_video || INTERRUPT_PENDING & 0x2)) {
	thread_to_try->waiting = 0;
...
/*
For all threads currently running, release any threads waiting on the current thread
*/
void clearDependencies(int32_t thread_id) {
...
/*
Function that gets called when a thread returns from its function.

If the thread was an event handler:
- reset the thread slot so that proper round robin order in maintained
- restart a different thread
...
uint16_t finish_thread() {
  if (threads_status[current_thread_slot].event_handler) {
    ...
    int32_t thread_id = threads_status[current_thread_slot].id;
...
uint16_t resume_next_thread() {
  /* Check to make sure no event needs handled, then resume the next thread in line
     ...
     - The event handlers get handled in order
     - Once event handlers are satisfied, we then restart the next normal thread
  */
  uint32_t cont_value;
...
// For finding an open thread slot for new threads
...
// When making new thread, make sure the id assigned isn't already in use
uint16_t no_id_conflicts() {
  for (int i=0;i<max_threads;++i) {
    if (threads_status[i].id == next_thread_id) return 0;
  }
  return 1;
}

// Get a new id for a new thread
int32_t get_next_thread_id() {
    while (1) {
      next_thread_id = (next_thread_id + 1) > 0 ? (next_thread_id + 1) : 0;
      if (no_id_conflicts()) break;
    }
...
uint16_t new_thread(void (*f)()) {
    ...
    */
    ...
    thread_status_t* thread_info = &threads_status[thread_slot];
    thread_info->id = get_next_thread_id();
...
// Function to first enter the cartidge code
...
// Does this thread already exists -  duplicated functionality of no_id_conflicts()
uint16_t thread_id_exists(int32_t tid) {
  for (uint16_t i=0; i<max_threads;++i) {
...
// syscall function
void join (int32_t thread_to_join) {
  if (thread_to_join == -1) return;
  if (thread_id_exists(thread_to_join)) {
